...
def process_context_menu_actions(df=None):
    ...
    if 'change_baseline' in query_params:
        baseline_name = urllib.parse.unquote(query_params.get('change_baseline'))
        empreendimento = urllib.parse.unquote(query_params.get('empreendimento'))
        
        # Carrega os dados da baseline
        baseline_data = load_baseline_data(empreendimento, baseline_name)
        
        if baseline_data:
            # Salva no session_state para ser usado na próxima execução do Streamlit
            st.session_state.current_baseline = baseline_name
            st.session_state.current_baseline_data = baseline_data
            st.session_state.current_empreendimento = empreendimento
            print(f"✅ SUCESSO: Baseline '{baseline_name}' carregada para visualização.")
        else:
            st.session_state.current_baseline = None
            st.session_state.current_baseline_data = None
            st.session_state.current_empreendimento = None
            print(f"❌ ERRO: Baseline '{baseline_name}' não encontrada.")
            
        # Limpa params para não repetir na próxima carga
        st.query_params.clear()
        st.rerun() # Força o Streamlit a recarregar com o novo estado de sessão
...
def take_gantt_baseline(df, empreendimento, tipo_visualizacao):
    """Cria uma linha de base do estado atual do Gantt, salvando as datas REAIS como PREVISTAS da baseline."""
    ...
    try:
        ...
        if df_emp.empty:
            ...
            raise ValueError("Nenhum dado encontrado para o empreendimento selecionado")

        # 2. A 'foto' da baseline são as datas REAIS atuais
        baseline_data = {}
        for _, row in df_emp.iterrows():
            etapa = row["Etapa"]
            # Salva a data real como se fosse a 'prevista' da baseline
            baseline_data[etapa] = {
                "Inicio_Prevista": row.get("Inicio_Real"),
                "Termino_Prevista": row.get("Termino_Real")
            }

        # 3. Converte para um formato serializável (string)
        serializable_data = {}
        for etapa, datas in baseline_data.items():
            serializable_data[etapa] = {
                "Inicio_Prevista": datas["Inicio_Prevista"].strftime('%Y-%m-%d') if pd.notna(datas["Inicio_Prevista"]) else None,
                "Termino_Prevista": datas["Termino_Prevista"].strftime('%Y-%m-%d') if pd.notna(datas["Termino_Prevista"]) else None
            }

        # 4. Lógica para nomear a baseline (P1, P2, etc.)
        baselines = load_baselines()
        emp_baselines = baselines.get(empreendimento, {})
        
        # Filtra apenas as baselines do tipo "Gantt" para a contagem
        gantt_baselines = {k: v for k, v in emp_baselines.items() if v.get('tipo_visualizacao') == 'Gantt'}
        
        # Encontra o maior número Px existente
        max_p_num = 0
        for name in gantt_baselines.keys():
            if name.startswith('P') and name[1:].isdigit():
                max_p_num = max(max_p_num, int(name[1:]))
                
        next_version_num = max_p_num + 1
        version_name = f"P{next_version_num}"
        created_date = datetime.now().strftime("%d/%m/%Y")

        # 5. Salva no banco de dados
        if save_baseline(empreendimento, version_name, serializable_data, created_date, tipo_visualizacao):
            return version_name
        else:
            raise Exception("Falha ao salvar a baseline no banco de dados.")
...
def apply_baseline_to_dataframe(df, baseline_data):
    """
    Aplica os dados da baseline ao DataFrame principal, substituindo as datas
    'Inicio_Prevista' e 'Termino_Prevista' pelas datas da baseline.
    A baseline_data é um dicionário {etapa: {'Inicio_Prevista': 'YYYY-MM-DD', 'Termino_Prevista': 'YYYY-MM-DD'}}
    """
    if not baseline_data or not isinstance(baseline_data, dict):
        return df
    ...
    # Iterar sobre as etapas na baseline_data
    for etapa, datas in baseline_data.items():
        # Encontrar a linha correspondente no DataFrame
        # Como o df já deve estar filtrado pelo empreendimento, a máscara por etapa deve ser suficiente.
        mask = (df_baseline['Etapa'] == etapa)
        
        if mask.any():
            
            # Atualizar datas previstas da baseline
            inicio_previsto_baseline = datas.get('Inicio_Prevista')
            termino_previsto_baseline = datas.get('Termino_Prevista')
            
            # Aplicar Inicio_Prevista
            if inicio_previsto_baseline:
                df_baseline.loc[mask, 'Inicio_Prevista'] = pd.to_datetime(inicio_previsto_baseline)
            else:
                df_baseline.loc[mask, 'Inicio_Prevista'] = pd.NaT
                
            # Aplicar Termino_Prevista
            if termino_previsto_baseline:
                df_baseline.loc[mask, 'Termino_Prevista'] = pd.to_datetime(termino_previsto_baseline)
            else:
                df_baseline.loc[mask, 'Termino_Prevista'] = pd.NaT
                
            # As colunas Inicio_Real e Termino_Real permanecem inalteradas.
...
def gerar_gantt(df, tipo_visualizacao, filtrar_nao_concluidas, df_original_para_ordenacao, pulmao_status, pulmao_meses, etapa_selecionada_inicialmente, baseline_data=None, baseline_name=None):
    ...
    should_apply_baseline = (
        st.session_state.get('current_baseline_data') is not None and 
        st.session_state.get('current_baseline') is not None and 
        current_empreendimento == empreendimento_visualizado and
        empreendimento_visualizado != "Múltiplos"
    )
    
    if should_apply_baseline:
        baseline_name = st.session_state.current_baseline
        baseline_data = st.session_state.current_baseline_data
        
        # Aplica a baseline ao DataFrame. Isso substitui Inicio_Prevista e Termino_Prevista
        ...
        with st.container():
            ...
            with col1:
                st.info(f"**Visualizando Baseline:** {baseline_name} | **Projeto:** {empreendimento_visualizado}")
            with col2:
