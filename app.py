...
def gerar_gantt_consolidado(df, tipo_visualizacao, df_original_para_ordenacao, pulmao_status, pulmao_meses, etapa_selecionada_inicialmente):
    ...
    # === CARREGAR BASELINES PARA O CONSOLIDADO ===
    all_baselines = load_baselines()  # Carrega todas as baselines do banco
    
    # Preparar estrutura de baselines para JavaScript
    baselines_por_empreendimento = {}
    baseline_options_por_emp = {}
    
    for emp in empreendimentos_no_df:
        if emp in all_baselines:
            emp_baselines = all_baselines[emp]
            
            # Converter baseline_data (JSON string) para dict
            baselines_dict = {}
            baseline_names = []
            
            for baseline_name, baseline_info in emp_baselines.items():
                baseline_names.append(baseline_name)
                
                # Parse baseline_data se for string JSON
                if isinstance(baseline_info.get('baseline_data'), str):
                    try:
                        baseline_data_parsed = json.loads(baseline_info['baseline_data'])
                    except:
                        baseline_data_parsed = baseline_info.get('baseline_data', {})
                else:
                    baseline_data_parsed = baseline_info.get('baseline_data', {})
                
                # Extrair tasks da baseline
                tasks_list = baseline_data_parsed.get('tasks', [])
                
                # Organizar por etapa
                etapas_dict = {}
                for task_bl in tasks_list:
                    etapa_sigla = task_bl.get('etapa', '')
                    etapa_nome = sigla_para_nome_completo.get(etapa_sigla, etapa_sigla)
                    
                    etapas_dict[etapa_nome] = {
                        'start': task_bl.get('inicio_previsto'),
                        'end': task_bl.get('termino_previsto')
                    }
                
                baselines_dict[baseline_name] = {
                    'created_date': baseline_info.get('created_date', ''),
                    'etapas': etapas_dict
                }
            
            # Identificar última baseline (maior número P)
            latest_baseline = None
            if baseline_names:
                # Ordenar por número (P1, P2, P3...)
                def extract_number(name):
                    import re
                    match = re.search(r'P(\d+)', name)
                    return int(match.group(1)) if match else 0
                
                baseline_names_sorted = sorted(baseline_names, key=extract_number, reverse=True)
                latest_baseline = baseline_names_sorted[0]
            
            baselines_por_empreendimento[emp] = {
                'baselines': baselines_dict,
                'latest': latest_baseline,
                'options': baseline_names
            }
            baseline_options_por_emp[emp] = baseline_names
        else:
            baselines_por_empreendimento[emp] = {
                'baselines': {},
                'latest': None,
